...
int nAnonymizeBitCurrencyProAmount = 1000;
...
#else
    const char* pszModule = "BitCurrencyPro";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitCurrencyPro
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitCurrencyPro
    // Mac: ~/Library/Application Support/BitCurrencyPro
    // Unix: ~/.BitCurrencyPro
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitCurrencyPro";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BitCurrencyPro.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BitCurrencyProd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitCurrencyPro will not work properly.");
