...
void Shutdown()
{
    ...
    RenameThread("BitCurrencyPro-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: BitCurrencyPro.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: BitCurrencyProd.pid)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 16815)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitCurrencyProamount=<n> " + _("Keep N BitCurrencyPro anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitCurrencyPro is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitCurrencyPro is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitCurrencyPro version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "BitCurrencyPro server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitCurrencyPro") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BitCurrencyPro to complete") << "\n";
    ...
    nAnonymizeBitCurrencyProAmount = GetArg("-anonymizeBitCurrencyProamount", 0);
    if(nAnonymizeBitCurrencyProAmount > 999999) nAnonymizeBitCurrencyProAmount = 999999;
    if(nAnonymizeBitCurrencyProAmount < 2) nAnonymizeBitCurrencyProAmount = 2;
    ...
    LogPrintf("Anonymize BitCurrencyPro Amount %d\n", nAnonymizeBitCurrencyProAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BTCRP+1000 == (.1BTCRP+100)*10
       10BTCRP+10000 == (1BTCRP+1000)*10
    */
