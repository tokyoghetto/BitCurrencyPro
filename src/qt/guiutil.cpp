...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(10);
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("BitCurrencyPro"))
        return false;
...
{
    // Convert BitCurrencyPro:// to BitCurrencyPro:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BitCurrencyPro://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "BitCurrencyPro:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitCurrencyPro.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BitCurrencyPro.lnk
...
{
    return GetAutostartDir() / "BitCurrencyPro.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a BitCurrencyPro.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BitCurrencyPro\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BitCurrencyPro app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BitCurrencyPro app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitCurrencyPro-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BitCurrencyPro-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitCurrencyPro-Qt"));
