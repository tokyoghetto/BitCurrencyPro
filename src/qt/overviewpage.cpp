...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitCurrencyProAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitCurrencyProAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitCurrencyProAmount = strAnonymizeBitCurrencyProAmount.remove(strAnonymizeBitCurrencyProAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitCurrencyProAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitCurrencyProAmount*COIN) nMaxToAnonymize = nAnonymizeBitCurrencyProAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitCurrencyProAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitCurrencyProAmount));
        strAnonymizeBitCurrencyProAmount = strAnonymizeBitCurrencyProAmount.remove(strAnonymizeBitCurrencyProAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitCurrencyProAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBitCurrencyProAmount == 0){
