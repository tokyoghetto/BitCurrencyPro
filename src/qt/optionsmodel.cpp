...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitCurrencyProAmount"))
        settings.setValue("nAnonymizeBitCurrencyProAmount", 1000);
    nAnonymizeBitCurrencyProAmount = settings.value("nAnonymizeBitCurrencyProAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitCurrencyProAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBitCurrencyProAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitCurrencyProAmount:
            return QVariant(nAnonymizeBitCurrencyProAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitCurrencyProAmount:
            nAnonymizeBitCurrencyProAmount = value.toInt();
            settings.setValue("nAnonymizeBitCurrencyProAmount", nAnonymizeBitCurrencyProAmount);
            emit AnonymizeBitCurrencyProAmountChanged(nAnonymizeBitCurrencyProAmount);
            break;
        default:
            break;
        }
